...
- **Amazon Transcribe**
  ...
  - **커스텀 어휘(Custom Vocabulary)**: 브랜드명, 약어, 고유명사, 도메인 특화 용어처럼 Transcribe가 잘 렌더링하지 못하는 **특정 단어들의 전사 정확도를 개선**하는 기능. 모든 지원 언어에서 사용 가능하며, 해당 언어의 문자 집합(character set)에 있는 문자만 사용할 수 있습니다.
    - **제한 사항**:
      - AWS 계정당 최대 **100개**의 커스텀 어휘 파일
      - 파일당 크기 제한 **50 KB**
      - 항목당 최대 **256자**
      - 전사(transcription)와 **동일한 AWS 리전**에서 생성되어야 함
    - **파일 형식**:
      - **API**로 생성 시 → 텍스트(`*.txt`) 형식만 가능
      - **AWS Management Console**로 생성 시 → 텍스트(`*.txt`) 또는 CSV(`*.csv`) 형식 가능
    - **⚠️ 데이터 무결성**: 커스텀 어휘에 **기밀정보, PII(개인식별정보), PHI(보호대상 건강정보) 입력 금지**.
    - **테이블(Table) vs 리스트(List)**:
      - **테이블 형식**은 커스텀 어휘의 입력과 출력을 세밀하게 제어할 수 있도록 여러 카테고리(`Phrase`, `DisplayAs` 등)를 지정하는 방식입니다.
      - **리스트 형식은 사용 중단(deprecated) 예정**이며, 새 커스텀 어휘를 만들 때는 **테이블 형식을 사용**해야 합니다.
      - AWS Management Console, AWS CLI, AWS SDK 모두 테이블 방식은 동일하게 동작하지만, 리스트 방식은 방법마다 다르게 사용됩니다.
    - **관련 API 작업**: `CreateVocabulary`, `DeleteVocabulary`, `GetVocabulary`, `ListVocabularies`, `UpdateVocabulary`.
- **Amazon Polly**
...
>   - **Custom Vocabulary**: Improves transcription accuracy for **specific words** (like brand names, acronyms, proper nouns, and domain-specific terms) that Amazon Transcribe isn't rendering correctly. Available for all supported languages; only characters in the language's character set can be used.
>     - **Limits**:
>       - Up to **100 custom vocabulary files** per AWS account
>       - Size limit: **50 KB** per file
>       - Up to **256 characters** per entry
>       - Must be created in the **same AWS Region** as your transcription
>     - **File Format**:
>       - Via **API** → text (`*.txt`) format only
>       - Via **AWS Management Console** → text (`*.txt`) or CSV (`*.csv`) format
>     - **⚠️ Data Integrity**: **Do NOT enter confidential information, PII, or PHI** into a custom vocabulary.
>     - **Tables vs Lists**:
>       - **Table format** gives fine-grained control with multiple categories (`Phrase`, `DisplayAs`) for input/output.
>       - **List format is being deprecated** — use the table format when creating a new custom vocabulary.
>       - Tables behave the same across AWS Management Console, AWS CLI, and AWS SDKs; lists are used differently across methods.
>     - **Related API operations**: `CreateVocabulary`, `DeleteVocabulary`, `GetVocabulary`, `ListVocabularies`, `UpdateVocabulary`.
> - **Amazon Polly**
...
- **SageMaker JumpStart**
  ...
  - **파운데이션 모델(Foundation Models)**: JumpStart는 콘텐츠 작성, 코드 생성, 질의응답, 카피라이팅, 요약, 분류, 정보 검색 등 다양한 유즈 케이스에 사용할 수 있는 **최신 파운데이션 모델(FM)**을 제공합니다. 파운데이션 모델은 대규모 사전학습 모델로서, 여러 다운스트림(downstream) 작업에 적응 가능하며 더 특화된 모델 개발의 **출발점** 역할을 합니다.
    - **모델 예시**: **LLaMa-3-70b**, **BLOOM 176B**, **FLAN-T5 XL**, **GPT-J 6B** 등 방대한 텍스트 데이터로 사전학습되어 특정 언어 작업에 맞게 파인튜닝할 수 있는 모델.
    - **모델 소스**: 
      - **공개 파운데이션 모델**: JumpStart가 온보딩·유지 관리하여 접근·커스터마이징·통합할 수 있게 해 줍니다.
      - **서드파티 제공자의 독점(proprietary) 파운데이션 모델**도 포함됩니다.
    - **접근 방식**: 모든 파운데이션 모델은 **SageMaker Python SDK**를 통해 프로그래밍 방식으로 사용할 수 있습니다.
    - **커스터마이징·파인튜닝**: JumpStart의 파운데이션 모델은 자체 데이터로 커스터마이징하고 파인튜닝할 수 있습니다 (Foundation model customization).
    - **모델 평가**: **Studio에서 텍스트 생성 파운데이션 모델을 평가**할 수 있어, 특정 유즈 케이스에 가장 적합한 모델을 선택할 수 있습니다.
    - **모델 선택 시 고려 사항**: 각 모델의 **라이선스 계약(license agreements)**과 **모델 소스**를 확인하는 것이 중요합니다.
- **SageMaker Clarify**
...
>   - **Foundation Models**: JumpStart offers **state-of-the-art foundation models (FMs)** for use cases such as content writing, code generation, question answering, copywriting, summarization, classification, and information retrieval. A foundation model is a large pre-trained model adaptable to many downstream tasks and often serves as the **starting point for developing more specialized models**.
>     - **Example Models**: **LLaMa-3-70b**, **BLOOM 176B**, **FLAN-T5 XL**, **GPT-J 6B** — all pre-trained on massive amounts of text data and can be fine-tuned for specific language tasks.
>     - **Model Sources**:
>       - **Publicly available foundation models** — JumpStart onboards and maintains these for you to access, customize, and integrate.
>       - **Proprietary foundation models** from third-party providers are also available.
>     - **Access**: All foundation models are available to use **programmatically with the SageMaker Python SDK**.
>     - **Customization & Fine-Tuning**: JumpStart foundation models can be customized and fine-tuned with your own data (Foundation model customization).
>     - **Model Evaluation**: You can **evaluate text generation foundation models in Studio**, helping you select the best model for your use case.
>     - **Choosing a Model**: Review **model sources and license agreements** carefully when selecting a foundation model.
> - **SageMaker Clarify**
